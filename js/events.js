...
export function setupEvents() {
  document.getElementById('btn-add').addEventListener('click', () => {
    ...
    state.stages.push({
      ...
      tankPressure: PROPS[prop].defaultPressure,
      burnTime: 180,
    });
    renderCards();
    update();
  ...
  document.getElementById('stage-cards').addEventListener('input', e => {
    ...
    else if (param === 'tankPressure'){ labelId = `v-pressure-${id}`; labelText = (+el.value) + ' bar'; }
  ...
  // Change handler for <select> elements (propellant + tank material)
  document.getElementById('stage-cards').addEventListener('change', e => {
    ...
    if (param === 'prop') {
      ...
      s.tankPressure = PROPS[el.value].defaultPressure;
      renderCards();
